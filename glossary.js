...
{target:`正規表現例`, content:`[A-Za-z0-9] → 大文字・小文字・数字<br>[^A-Za-z0-9] → 大文字・小文字・数字以外<br>&yen;s → 空白、タブ、改ページ、改行、その他の Unicode 空白文字などホワイトスペース 1 文字に一致<br>&yen;S → ホワイトスペース以外の 1 文字に一致<br>x＊→ xの0回以上の繰り返しに一致<br>x+ → xの1回以上の繰返しに一致<br>x{n} → xがn回出現に一致<br>^と$ → 先頭と末尾<br>/^[A-Za-z0-9]*$/.test('ABCabc123'); → true 大文字・小文字・数字のみをチェック<br>'Abc 123!!456'.replace(/[^A-Za-z0-9&yen;s]+/g, ' '); → 'Abc 123456' 大文字、小文字、数字、スペース以外の文字をスペースに変換<br>text.replace(/&yen;s+/g, ' '); → 1つ以上連続する空白文字（改行、タブ、スペースなど）をスペースに変換`},
