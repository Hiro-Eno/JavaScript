...
{target:`関数（functions）`, content:`タスクを実行したり値を計算したりする一連の文。主なパターンとして①値を返す。②副作用を作り出す。③値を返し、副作用も作り出す。<br>function square(x){<br>return x * x;<br>}`},
{target:`関数の宣言`, content:`プログラムで関数を定義するための宣言手法。<br>function square(x){<br>return x * x;<br>}`},
...
{target:`値としての関数`, content:`関数は値でもあるので、他の関数に引数として渡すことが可能。引数として渡される関数はしばしば、コールバック関数と呼ばれる。<br>const square = function(x){return x * x}`},
{target:`副作用`, content:`本来の作用（返り値を返す）以外の動き。スコープ外のなんらかの状態を変えること。`},
{target:`コールバック関数`, content:`他の関数に引数として渡される関数。<br>const square = function(x){return x * x}`},
...
{target:`無名関数`, content:`名前を持たない関数。通常、関数式として変数に代入されたり、高階関数の引数として渡されたりする。<br>(x) => x * x `},
{target:`API（Application Programing Interface）`, content:`ユーザーが色々なデータを利用できるようになる仕組み。`},
{target:`アロー関数`, content:`通常の関数式を簡潔に記述するための構文。(引数) => 返り値。(引数) => {return 返り値}。(引数1, 引数2) => 返り値。() => 返り値 など。<br>(x) => {<br> return x * x;<br>}<br>(x) => x * x<br>x => x * x`},
