...
{target:`forループ`, content:`括弧で囲みセミコロンで区切った 3 つの引数と、続いてループ内で実行される文（ふつうはブロック文）から成るループを構成。if文と同じような働きをし、丸括弧()のなかで変数（通常i, j, k）の初期化、条件、条件がいずれfalseになる処理を書く（それぞれはセミコロン';'で区切る）。条件がtrueである限り、波括弧{}の中のループの処理が繰り返される。<br>for (let i = 0; i < 9; i++) {<br>}`},
...
{target:`カプセル化`, content:`データと関数を一つのコンポーネントに閉じ込め、そのコンポーネントへのアクセスを制御することにより、そのオブジェクトを 'ブラックボックス' にすること。ユーザーは隠れた実装ではなくインターフェースを知っているだけで済む。`},
...
{target:`スキーマ`, content:`データベースの構造を定義したもので、データベース管理システム（DBMS）でサポートされる形式言語で記述。関係データベースでは、スキーマは関係（表）とその中の属性（フィールド）や、属性と関係の関連を定義。`},
