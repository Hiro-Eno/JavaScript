...
{target:`ブロック`, content:`プログラム内の複数のステートメントやコード行をまとめ、1つのまとまりとして扱うためのコードの集まり。通常、中括弧{}で囲まれ、その中に複数のコード行が含まれる。`},
...
{target:`条件 (三項) 演算子`, content:`3 つのオペランドをとる演算子。条件に続いて疑問符 (?)、そして条件が真値であった場合に実行する式（返り値）、コロン (:) が続き、条件が偽値であった場合に実行する式（返り値）が最後に来る。<br>condition ? exprIfTrue : exprIfFalse;<br>sell < 100 ? {tip = '$2.00'} : {tip='$10.00'};<br><br>condition ? rvTrue : rvFalse;<br>sell < 100 ? '$2.00' : '$10.00';`},
...
{target:`for-inループ`, content:`キーが文字列であるオブジェクトの列挙可能プロパティすべてに対して、継承された列挙可能プロパティも含めて反復処理する。波括弧{}の中でループ処理をする。値は通常ブラケット記法でアクセスする。<br>for (const key in obj){<br>}`},
{target:`for-ofループ`, content:`配列の各要素にアクセスし一連の値を処理するループを実行。波括弧{}の中でループ処理をする。<br>for (const of array){<br>}`},
{target:`forループ`, content:`括弧で囲みセミコロンで区切った3つの引数と、続いてループ内で実行される文（ふつうはブロック文）から成るループを構成。if文と同じような働きをし、丸括弧()のなかで変数（通常i, j, k）の初期化、条件、条件がいずれfalseになる処理を書く（それぞれはセミコロン';'で区切る）。条件がtrueである限り、波括弧{}の中のループの処理が繰り返される。<br>for (let i = 0; i < 9; i++) {<br>}`},
{target:`whileループ`, content:`テスト条件が真と評価されている間、指定された文を実行するループを作成。条件はその文を実行する前に評価する。if文と同じような働きをし、丸括弧()の中の条件がtrueである限り、波括弧{}の中のループの処理が繰り返される。<br>let n = 0;<br>while (n < 3) {<br>&emsp;n++;<br>}`},
...
{target:`カプセル化`, content:`データと関数を一つのコンポーネントに閉じ込め、そのコンポーネントへのアクセスを制御することにより、そのオブジェクトをブラックボックスにすること。ユーザーは隠れた実装ではなくインターフェースを知っているだけで済む。`},
...
{target:`Array.prototype`, content:`JavaScript の配列オブジェクト (Array) のプロトタイプを指す。すべての配列インスタンスが共通して使用できるメソッドやプロパティを定義。`},
...
{target:`アロー関数`, content:`通常の関数式を簡潔に記述するための構文。(引数) => 返り値。(引数) => {return 返り値}。(引数1, 引数2) => 返り値。() => 返り値 など。<br>const func = (x) => {<br>&emsp;return x * x;<br>}<br>(x) => x * x;<br>x => x * x;`},
{target:`スプレッド構文（...）`, content:`配列やオブジェクト、文字列を展開する演算子。すべての要素を何らかのリストに入れる必要がある場合に使用することができる。また、slice()のように配列を複製やconcat()のように配列を連結できる。残余構文（引数）と外見は似ているが、逆のはたらき。この構文は要素を展開するのに対して、残余構文は複数の要素を集約して1つのオブジェクトに「濃縮」する。<br>const arr = [1, 2, 3]<br>[...arr, 4, 5] → [1, 2, 3, 4, 5]<br>Math.max(...arr) → 3<br>func.apply(null, arr) → func(...arr)（apply()の同様の働き）<br>let arr2 = [...arr]（新しい配列への複製）<br>arr2 = arr2.concat(arr) → arr2 = [...arr, ...arr2]（concat()と同様の働き）<br>[...Array(5)] → undefined×5の配列を作成（繰り返し処理に利用可能）<br><br>let obj1 = { foo: 'bar', x: 42 };<br>let obj2 = { foo: 'baz', y: 13 };<br>let clonedObj = { ...obj1 } → Object { foo: 'bar', x: 42 }<br>let mergedObj = { ...obj1, ...obj2 } → Object { foo: 'baz', x: 42, y: 13 }`},
...
{target:`this`, content:`現在のオブジェクトを指すキーワード。値はどのように関数が呼ばれたかによって決定される (実行時結合)。これは実行時に代入によって設定することはできず、関数が呼び出されるたびに異なる可能性があるので注意が必要。`},
{target:`ファクトリ関数`, content:`オブジェクトを生成して返す関数。同じプロパティやメソッドを持つ複数のオブジェクトを簡単に生成することができる。オブジェクトの作成と初期化の責任を持ち、関数内で新しいオブジェクトを作成し、そのオブジェクトに必要なプロパティやメソッドを設定し、最終的にそのオブジェクトを返す。`},
{target:`イテレータ―`, content:`要素を一つずつ順番に処理するオブジェクト。`},
{target:`HTMLCollection`, content:`要素の汎用的な集合を表し、リストから選択するためのメソッドとプロパティを提供するインターフェース。名前とインデックスの両方で、自身のメンバーをプロパティとして直接公開。HTML DOM内のこれは生きて (live) いる。そのため、元になった文書が変更された時点で自動的に更新される。document.formsやdocument.getElementsByTagName('div')などで取得。`},
{target:`NodeList`, content:`ノードの集合であり、 Node.childNodes などのプロパティやdocument.querySelectorAll() などのメソッドの返値として用いられる。生きたものと静的なものの2種類がある。DOMが変化すると、自動的に集合が更新されることを生きたと呼ぶ。静的なものの場合、DOM内の変更が集合の内容に影響を与えない。document.querySelectorAll()メソッドは、静的なものを返す。`},
{target:`コンテキスト`, content:`実行される場所。thisの値に影響を与える。`},
{target:`関数コンテキスト`, content:`関数内でのthisの値は、関数の呼び出し方によって異なる。関数の呼び出し時にthisの値を特定の値に設定するには、call()またはapply()を使用する。`},
...
{target:`データモデリング`, content:`データの流れと管理システムを図とテキストで定義・可視化すること。データの「設計図」を作るプロセスであり、その設計図を実際に建物「データベース」に建てる工程。<br>より効率的で優れたデータ管理を促進することを目的とし、データを整理し、整え、ルールで管理することで、情報をより効果的に活用できるようになる。<br>まず、具体的なデータの要件や目的を把握、データをどのように構造化するかを決定（例：名前、住所、電話番号などの顧客テーブルを作成）。<br>次に、データの一貫性や整合性を保つための条件や規則など、データに適用される制約やルールを定義する（例：顧客ID、注文日は未来の日付など）。<br>これらの手順を踏んでデータモデルを作成し、データベース管理システム (DBMS) を使って実際にデータモデルを実装する。DBMSを使うことで、データを効率的に格納し、検索し、更新することができる。<br>データモデリング方法論とは、データを整理し、パターン化し、組織化するための手法やルールの集まりで、情報を理解しやすくし効率的な管理に役立つ。<br>データモデルのインスタンスには、概念スキーマ、論理スキーマ、物理スキーマの3種類ある。`},
{target:`データモデリングのプロセス`, content:`最初に概念データモデルを作成し、次に論理データモデル、最後に物理データモデルを作成。データを詳細に可視化してから、データベースの技術要件を指定できる。<br>①エンティティの特定：社内のさまざまなエンティティや部門を特定し、リストアップ<br>②各エンティティの主要なプロパティを特定：社内の各エンティティを区別する属性を特定<br>③エンティティ間の関係（リレーションシップ）を特定：相互に関連するエンティティを提示し、それらの関係を可視化するモデルを開発<br>④モデルに追加するデータ属性を特定：モデルに含める必要のある特定の属性（顧客のメールアドレスや名前など）をリストアップ<br>⑤属性のマッピング：データ属性を作成し、各エンティティにマッピング<br>⑥最終確認と正確性の検証：データモデルを確定する前に、意思決定者はその正確性を検証し、初期のフレームワークに関連するエンティティと属性がすべて含まれていることを確認`},
{target:`データモデリングの利点`, content:`収集したデータをより効率的に整理することができ、データを最大限に活用し、重要なビジネスプロセスをサポートすることができる。その他の利点として、データの一貫性、データ品質の向上、効率的なデータベース設計、エラーと冗長性の低減、企業全体の透明性の向上などがあげられる。`},
{target:`スキーマ`, content:`データベースの構造を定義したもので、データベース管理システム（DBMS）にサポートされる形式言語で記述。関係データベースでは、関係（表）とその中の属性（フィールド）や、属性と関係の関連を定義。`},
{target:`概念スキーマ・概念モデル・概念データモデル`, content:`概念と概念間の関係の定義。データベース設計における抽象的なレベルの表現でデータベース内の情報を組織化し、その構造や関係を表すためのモデル。データモデル全体の枠組みを提供し、組織内の異なる利用者やシステムが共通の理解を持つことを可能にする。データベース内の情報の本質的な構造を表現し、それがどのように関連付けられるかを明確にする。これにより、データベースがより柔軟で、変更に対応しやすくなる。`},
{target:`論理スキーマ・論理モデル・論理データモデル`, content:`実体とその属性、実体間の関係の定義。データベースの設計の概要を表す。特定のデータ管理技術を前提として対象領域を記述するデータモデルだが、特定のデータベース管理製品に依存することなく、汎用的なデータ構造を用いて表現する。データベース内の情報をどのように構造化し、関連付けるかを示す設計図のようなもの。<br>論理スキーマが「何を」するかを示すのに対し、物理スキーマは「どのように」するかを示す。`},
{target:`物理スキーマ・物理モデル・物理データモデル`, content:`論理スキーマの具体的実装。特定のデータ管理技術の観点で記述されるデータモデル。具体的なデータベース管理製品に依存し、データがどのようにストレージ（補助記憶装置）に格納されるかを詳細に定義。論理スキーマで定義されたデータの論理的な構造を、物理的なストレージにどのように配置するかを示す。<br>論理スキーマが「何を」するかを示すのに対し、物理スキーマは「どのように」するかを示す。<br>定義される要素にはフィールド、列、インデックス、トリガー、制約があげられる。`},
{target:`関係データモデリング`, content:`複数のテーブルに格納されたデータ要素の関係をマッピングする手法。物理データの格納状況を詳細に把握する必要がないため、多くのモデラーが採用。`},
{target:`ERデータモデリング`, content:`エンティティをマッピングし、これらのエンティティに関連する特定の属性を識別するための、より複雑な関係データモデル。エンティティには、顧客データ、従業員データ、製品情報、請求書などが含まれる。属性には、顧客のメールアドレス、従業員の姓、製品価格、請求書の作成日などが含まる。<br>エンティティは長方形で表され、属性は楕円で表され、エンティティ間の関係は矢印で表され、関連の性質や種類が記述される。`},
{target:`エンティティ`, content:`データベース内で重要な情報のまとまりを表す用語。データベース内の「何か」を表し、顧客、商品、注文、従業員などを示す。<br>顧客エンティティは実際のお店やウェブサイトで商品を購入する人々の集まりを、商品エンティティはそのお店やウェブサイトで販売されている実際の商品を表す。<br>各エンティティはその特徴や詳細を示す関連する情報や属性を持つ。顧客エンティティには名前、住所、電話番号などの属性が、商品エンティティには商品名、価格、在庫数などの属性がある。`},
{target:`階層型データモデリング`, content:`データの親子関係をツリー状に表現したもの。`},
{target:`次元データモデリング`, content:`企業がデータウェアハウスに保存された情報に効率的にアクセスできるように、データ検索スピードを最適化するように設計。主にBI（ビジネスインテリジェンス）アプリケーションのデータマートやデータウェアハウスで使用され、製品の購入などのイベントやトランザクションに関する情報を格納するファクトテーブルを使用。`},
{target:`オブジェクト指向データモデリング`, content:`エンティティをオブジェクトとして抽象化。同じ属性を共有するオブジェクトをクラスに移動し、階層的に分類する。ERモデリングと類似。`},
