...
{target:`メソッド`, content:`オブジェクトのプロパティの値が関数のもの（オブジェクトは関数も格納可）。オブジェクトインスタンスごとに内蔵されたタスクとして実行されるインスタンスメソッドと、オブジェクトのコンストラクターで直接呼び出しを行うタスクである静的メソッドの2種類がある。<br>const obj = {<br>&emsp;foo: function () {<br>&emsp;},<br>&emsp;bar: function () {<br>&emsp;},<br>};<br>簡略型<br>const obj = {<br>&emsp;foo() {<br>&emsp;},<br>&emsp;bar() {<br>&emsp;},<br>};`},
