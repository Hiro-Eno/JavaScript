...
{target:`条件分岐 conditionals`, content:`プログラム内である条件が満たされた場合や特定の状況に応じて、異なるコードブロックや処理が実行されるプログラムの構造`},
{target:`if文`, content:`if(){}else if(){} else {}`},
...
{target:`for-ofプール`, content:`配列の各要素にアクセス。配列の各要素が代入される変数の宣言。波括弧{}の中でループ処理をする`},
...
{target:`クロージャ`, content:`関数が周囲のスコープにある変数を使って作成されると、その変数は関数と組合され、関数は周囲のスコープを作っていた外側の関数が完了した後でも、その変数を利用できる。活用例としてデータプライバシー、同じような関数をたくさん作る、`},
{target:`データプライバシー`, content:`グローバルスコープからアクセスできないように全体をスコープに入れて作る。そうするとローカル変数はグローバルスコープからアクセスできなくなる`},
{target:`メソッド`, content:`オブジェクトには関数も格納できる。プロパティの中で値が関数のものをメソッドと呼ぶ`},
...
{target:`Lodash`, content:`オープンソースライブラリ。'_.'で呼び出し`},
...
{target:`param  パラメータ`, content:`関数やメソッドに渡される引数のこと。関数が実行される際に、パラメータは関数内で使用される変数として扱われる`},
...
{target:`argument  引数`, content:`関数やメソッドに渡される値。パラメータとして関数内で使用される`},
...
{target:`arg`, content:`引数の略称。関数やメソッドに渡される値として利用`},
